...
class ControlsSubState extends MusicBeatSubstate
{
	...
	var bg:FlxSprite;
	var grpDisplay:FlxTypedGroup<FlxTextAlphabet>;
	var grpBlacks:FlxTypedGroup<AttachedSprite>;
	var grpOptions:FlxTypedGroup<FlxTextAlphabet>;
	var grpBinds:FlxTypedGroup<FlxTextAlphabet>;
	...
	public function new()
	{
		...
		grpDisplay = new FlxTypedGroup<FlxTextAlphabet>();
		add(grpDisplay);
		...
		add(selectSpr);
		grpBinds = new FlxTypedGroup<FlxTextAlphabet>();
		add(grpBinds);
		...
		var text:FlxTextAlphabet = new FlxTextAlphabet(40, 110, 'CTRL', 40);
		text.setFormat(Paths.font("disneyFreeplayFont.ttf"), 40, FlxColor.WHITE, CENTER, FlxTextBorderStyle.OUTLINE, FlxColor.BLACK);
		add(text);

		createTexts();

		controlsTxt = new FlxText(0, FlxG.height * 0.05, 0, 'Controls');
	...
	function createTexts()
	{
		curOptions = [];
		curOptionsValid = [];
		grpDisplay.forEachAlive(function(text:FlxTextAlphabet) text.destroy());
		grpBlacks.forEachAlive(function(black:AttachedSprite) black.destroy());
		grpOptions.forEachAlive(function(text:FlxTextAlphabet) text.destroy());
		grpBinds.forEachAlive(function(text:FlxTextAlphabet) text.destroy());
		grpDisplay.clear();
		grpBlacks.clear();
		grpOptions.clear();
		grpBinds.clear();
		...
		for (i in 0...options.length)
		{
			...
			if(option[0] || onKeyboardMode)
			{
				if(option.length > 1)
				{
					...
					var text:FlxTextAlphabet = new FlxTextAlphabet(200, 300, 0, option[1], 80);
					text.isMenuItem = true;
					text.setFormat(Paths.font("BROUGHTTHESTYLE.otf"), 100, FlxColor.WHITE, CENTER, FlxTextBorderStyle.OUTLINE, FlxColor.BLACK);
					text.changeX = false;
	...
	function addCenteredText(text:FlxTextAlphabet, option:Array<Dynamic>, id:Int)
	{
		text.screenCenter(X);
		text.y -= 55;
		text.startPosition.y -= 55;
	}
	function addKeyText(text:FlxTextAlphabet, option:Array<Dynamic>, id:Int)
	{
		for (n in 0...2)
		{
			...
			var attach:FlxTextAlphabet = new FlxTextAlphabet(textX + 210, 300, 0, key, 80);
			attach.isMenuItem = true;
			attach.changeX = false;
			attach.setFormat(Paths.font("VanillaExtractRegular.ttf"), 70, FlxColor.WHITE, CENTER, FlxTextBorderStyle.OUTLINE, FlxColor.BLACK);
	...
	{
		var bind:FlxTextAlphabet = grpBinds.members[num];
		var attach:FlxTextAlphabet = new FlxTextAlphabet(350 + (num % 2) * 300, 300, 0, text, 80);
		attach.isMenuItem = true;
		attach.changeX = false;
		attach.setFormat(Paths.font("VanillaExtractRegular.ttf"), 70, FlxColor.WHITE, CENTER, FlxTextBorderStyle.OUTLINE, FlxColor.BLACK);
		attach.distancePerItem.y = 60;
	...
	var bindingText:FlxTextAlphabet;
	var bindingText2:FlxTextAlphabet;
	...
	override function update(elapsed:Float)
	{
		...
		controlsTxt.alpha = FlxMath.lerp(curSelected >= 3 ? .2 : 1, controlsTxt.alpha, .85);
	...
	function updateText(?move:Int = 0)
	{
		...
		grpDisplay.forEachAlive(function(item:FlxTextAlphabet) {
			item.targetY = item.ID - num - addNum;
		});

		grpOptions.forEachAlive(function(item:FlxTextAlphabet)
		{
			item.targetY = item.ID - num - addNum;
			item.alpha = (item.ID - num == 0) ? 1 : 0.6;
		});
		grpBinds.forEachAlive(function(item:FlxTextAlphabet)
		{
			var parent:FlxTextAlphabet = grpOptions.members[item.ID];
